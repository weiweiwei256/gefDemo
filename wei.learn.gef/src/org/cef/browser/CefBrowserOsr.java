...
class CefBrowserOsr extends CefBrowser_N implements CefRenderHandler {
	...
	CefBrowserOsr(CefClientHandler clientHandler, String url,
			boolean transparent, CefRequestContext context) {
		this(clientHandler, url, transparent, context, null, null);
	}

	private CefBrowserOsr(CefClientHandler clientHandler, String url,
			boolean transparent, CefRequestContext context,
	...
	public synchronized CefBrowser getDevTools(Point inspectAt) {
		if (devTools_ == null) {
			devTools_ = new CefBrowserOsr(clientHandler_, url_, isTransparent_,
					context_, this, inspectAt);
		}
		return devTools_;
	}
	...
	private void createGLCanvas() {
		...
		canvas_ = new GLCanvas(glcapabilities) {
			...
			public void paint(Graphics g) {
				if (parent_ != null) {
					createDevTools(parent_, clientHandler_, getWindowHandle(),
							isTransparent_, null, inspectAt_);
				} else {
					createBrowser(clientHandler_, getWindowHandle(), url_,
							isTransparent_, null, context_);
				}
				super.paint(g);
			}
		};
	...
	public void onPaint(CefBrowser browser, boolean popup,
		...
		renderer_.onPaint(canvas_.getGL().getGL2(), popup, dirtyRects, buffer,
				width, height);
