...
public class CefApp extends CefAppHandlerAdapter {
	...
	public final class CefVersion {
		...
		public String getJcefVersion() {
			return CEF_VERSION_MAJOR + "." + CHROME_VERSION_BUILD + "."
					+ JCEF_COMMIT_NUMBER;
		}
		...
		public String getChromeVersion() {
			return CHROME_VERSION_MAJOR + "." + CHROME_VERSION_MINOR + "."
				...
				public void run() {
					...
					if (!N_PreInitialize())
						throw new IllegalStateException(
								"Failed to pre-initialize native code");
				}
			};
	...
	public synchronized final void dispose() {
		...
		case INITIALIZED:
			...
			if (clients_.isEmpty()) {
				...
				// shutdown() will be called from clientWasDisposed() when the
				// last
				// client is gone.
				for (CefClient c : clients_) {
					c.dispose();
	...
	public synchronized CefClient createClient() {
		...
		default:
			throw new IllegalStateException("Can't crate client in state "
					+ state_);
		}
	...
	public boolean registerSchemeHandlerFactory(String schemeName,
		...
		try {
			return N_RegisterSchemeHandlerFactory(schemeName, domainName,
					factory);
		} catch (Exception err) {
			err.printStackTrace();
		}
	...
	final protected void clientWasDisposed(CefClient client) {
		...
		if (clients_.isEmpty()
				&& getState().compareTo(CefAppState.SHUTTING_DOWN) >= 0) {
		...
		try {
			Runnable r = new Runnable() {
				...
				public void run() {
					...
					System.out.println("initialize on "
							+ Thread.currentThread() + " with library path "
							+ library_path);
					...
					if (OS.isMacintosh()) {
						if (settings.browser_subprocess_path == null) {
							...
							settings.browser_subprocess_path = path.normalize()
						...
						if (settings.locales_dir_path == null)
							settings.locales_dir_path = library_path
									+ "/locales";
					}
	...
	protected final void handleBeforeTerminate() {
		// Execute on the AWT event dispatching thread. Always call
		// asynchronously
		// so the call stack has a chance to unwind.
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				CefAppHandler handler = (CefAppHandler) ((appHandler_ == null) ? this
						: appHandler_);
	...
	public final void doMessageLoopWork(final long delay_ms) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				// The maximum number of milliseconds we're willing to wait
				// between
	...
	private final String getJcefLibPath() {
		...
		library_path = System.getProperty("cef.path")
				+ System.getProperty("path.separator") + library_path;
		String[] paths = library_path.split(System
				.getProperty("path.separator"));
		String[] arg5 = paths;
		int arg4 = paths.length;

		for (int arg3 = 0; arg3 < arg4; ++arg3) {
			String path = arg5[arg3];
	...
	private final native boolean N_RegisterSchemeHandlerFactory(
			String schemeName, String domainName,
